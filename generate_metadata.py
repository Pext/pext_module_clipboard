...
for language in languages:
    if not language:
        ...
        try:
            lang = gettext.translation('pext_module_clipboard', localedir=os.path.join('.', 'locale'), languages=[language])
        except FileNotFoundError:
            lang = gettext.NullTranslations()
            print("No {} metadata translation available for pext_module_clipboard".format(language))
            continue
    ...
    json.dump({'id': 'pext.module.clipboard',
               'name': _('Clipboard'),
               'developer': 'Sylvia van Os',
               'description': _('Allows Pext to manage the clipboard'),
